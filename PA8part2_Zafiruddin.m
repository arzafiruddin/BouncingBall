...
% of the ball. (Part 2: Ameen's Video)
...
% BALL PROPERTIES
ballMass = 0.026; % ball mass (kg)
ballDIn = 8.50 / pi; % ball diameter (in)
ballDPx = 85; % ball diameter (px)
px2m = (ballDIn*0.0254) / ballDPx; % pixel to meter conversion factor
bounceCount = 4; % Number of clearly observed bounces

% VIDEO INFORMATION
vidFile = 'bounceAmeen.avi'; % file name of video (w/ extension)
frameStart = 63; % starting frame for analysis
frameStop = 117; % ending frame for analysis

% LOAD VIDEO
vid = VideoReader(vidFile); % reads in .avi file

% VIDEO CROPPING DIMENSIONS
xCropO = 790; % top-left x (col) point of crop
yCropO = 30; % top-left y (row) point of crop
xCropL = 400; % horizontal length of crop
yCropL = 1080; % vertical length of crop
aspRat = 2.63; % centroid plot aspect ratio value

% RGB THRESHOLDING VALUES
thUR = 80; % red upper threshold (0-255)
thLR = 0; % red lower threshold (0-255)
thUG = 255; % green upper threshold (0-255)
thLG = 20; % green lower threshold (0-255)
thUB = 255; % blue upper threshold (0-255)
thLB = 20; % blue lower threshold (0-255)
